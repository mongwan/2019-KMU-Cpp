...
위 코드 실행 시 위와 같은 창이 표시됩니다. 색상을 설정하기 위해 여러가지 시도를 해봤지만 (init_color()를 통해 다른 색상을 지정하는 등) 현재 제 터미널 환경에서는 변경이 반영되지 않아 색상 관련 코드는 최소화하였습니다. 열거체를 통해 COLOR_PAIR간의 구분이 더 잘 가도록 코드를 작성했습니다. 이중 for문을 통해 각 칸의 정보를 색으로 표현하였습니다.